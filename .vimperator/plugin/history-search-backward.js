...
(function() {
    ...
    let showCompletions = function() {
        if (!options.get('wildoptions').has('auto')) {
            evalWithContext(function() {
                ...
                commandline._completions.itemList.show();
            }, commandline.input);
        }
    }

    let next = function() {
        evalWithContext(function() commandline._completions.tab(false), commandline.input);
    }

    let prev = function() {
        evalWithContext(function() commandline._completions.tab(true), commandline.input);
    }
    ...
    mappings.addUserMap([modes.COMMAND_LINE], 
        ...
        {
            try{
            if (evalWithContext(function() commandline._completions.itemList.visible(), commandline.input)) {
                next();
                return;
            }
